...
//  MyMemo
//
//  Created by icoinnet on 2022/11/14.
//
...
extension ProfileVC {
    ...
    func touchID() {
        ...
        if context.canEvaluatePolicy(deviceAuth, error: &error){
            ...
            context.evaluatePolicy(deviceAuth, localizedReason: msg) {(success , e) in
                if success { // 5 인증 성공 : 토큰 갱씬 로직
                    ...
                    case LAError.systemCancel.rawValue:
                        self.alert("시스템에 의해 인증이 최소되었습니다.")
            ...
            //OperationQueue는 큐에 들어온 작업들이 순차적으로 처리되도록 해 주는 오브젝트-C 기반의 큐
            OperationQueue.main.addOperation(){
                self.commonLogout(true)
            }
    ...
    func commonLogout(_ isLogin: Bool = false) {
        //1. 저장된 기존 개인 정보 & 키 체인 데이터를 삭제하여 로그아룻 상태로 전환
...
class ProfileVC: UIViewController, UITableViewDelegate,  UITableViewDataSource, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    //UITableViewDataSource 프로토콜의 필수 메소드 2개
    ...
    @objc func doLogin(_ sender: Any){
        ...
        //인디케이터 실행
            ...
            self.uinfo.login(account: account, passwd: passwd, success: {
                ...
                //서버와 데이터 동기화
                let sync = DataSync()
                DispatchQueue.global(qos: .background).async { //백그라운드에서 실행
    ...
    @IBAction func backProfileVC(_ segue: UIStoryboardSegue){
        //단지 프로필 화면으로 되돌아오기 위한 표시 역할만 할 뿐이므로
        //아무 내용도 작성하지 않음

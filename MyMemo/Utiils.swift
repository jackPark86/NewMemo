...
extension UIViewController {
    ...
    private func sb(name: String) -> UIStoryboard {
        return UIStoryboard(name: name, bundle: Bundle.main)
    }
    
    func instanceTutorialVC(sbName: String,  storyboardId: String) -> UIViewController? {
        //return self.tutorialSB.instantiateViewController(withIdentifier: name)
        return self.sb(name: sbName).instantiateViewController(withIdentifier: storyboardId)
    }
    ...
    func alert(_ message: String, completion: (() -> Void)? = nil){
        ...
        DispatchQueue.main.async {
            ...
            let okAction = UIAlertAction(title: "확인", style: .cancel) { (_) in
                completion?() // completion 매개변수의 값이 nil이 아닐 때에민 실행
            }
...
class TokenUtils {
    ...
    var authorizationHearder : HTTPHeaders? {
        get {
            let serviceID = "kr.co.rubypaper.MyMemory"
            if let accessToken = self.load(serviceID, account: "accessToken") {
                return ["Authorization" : "Bearer \(accessToken)"] as HTTPHeaders
